...
ASD turns raw, messy, contradictory inputs into clean, versioned, interconnected, queryable
knowledge artifacts that serve as the Single Source of Truth.
...
Set `ASD_PROJECT_ROOT` to specify the project root (defaults to CWD). Set `ASD_KB_DIR` to override
the knowledge base directory (defaults to `USER/kb/`).
